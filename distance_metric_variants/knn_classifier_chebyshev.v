module knn_classifier #(
    ...
    localparam DIST_WIDTH = DATA_WIDTH + 1;
    ...
    reg [DIST_WIDTH-1:0] curr_distance;
    reg [DATA_WIDTH:0] diff1, diff2;

    // Chebyshev distance = max(abs(dx), abs(dy)).
    reg [DIST_WIDTH-1:0] nearest_dist0, nearest_dist1, nearest_dist2;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            test_feature2 <= 0;
            curr_distance <= 0;
            nearest_dist0 <= {DIST_WIDTH{1'b1}};
            nearest_dist1 <= {DIST_WIDTH{1'b1}};
            nearest_dist2 <= {DIST_WIDTH{1'b1}};
            nearest_label0 <= 1'b0;
            nearest_label1 <= 1'b0;
            nearest_label2 <= 1'b0;
            vote_class0 <= 0;
            ...
            case (state)
                ...
                CALC_DISTANCE: begin
                    if (i_reg < NUM_TRAINING) begin
                        diff1 = abs_diff(test_feature1, training_data1[i_reg]);
                        diff2 = abs_diff(test_feature2, training_data2[i_reg]);
                        curr_distance = (diff1 >= diff2) ? diff1 : diff2;
